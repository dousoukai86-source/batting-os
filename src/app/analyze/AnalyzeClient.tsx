...
import { getVideoObjectURL, loadVideo } from "@/lib/videoStore";
...
export default function AnalyzeClient({ type }: { type: CatNum }) {
  ...
  const [status, setStatus] = useState<
    "idle" | "loading_video" | "ready" | "analyzing" | "done"
  >("idle");
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      // フレーム抽出設定
      const sampleFps = 6; // 1秒あたり6枚サンプル（重ければ 3 に）
      ...
      for (let i = 0; i < totalFrames; i++) {
        ...
        // visibilityが低いフレームは捨てる（ノイズ減）
        if (visScore < 0.45) continue;

        const trunk = trunkLeanDeg(shoulderMid, hipMid);
        const hipAng = angleABC(picked.sh, picked.hip, picked.knee); // shoulder-hip-knee
        const kneeAng = angleABC(picked.hip, picked.knee, picked.ank); // hip-knee-ankle
...
function peakMetrics(list: PoseMetrics[]): PoseMetrics {
  // 体幹前傾が最大のフレームを “peak” として採用（あとで別ロジックに変えてOK）
  let best = list[0];
  for (const m of list) {
...
function buildMessage(type: CatNum, avg: PoseMetrics, peak: PoseMetrics) {
  // ここは “本気化の入口” ：今は角度に基づく一次評価
  // 後で：骨盤角/胸郭/頭部/踏み込みタイミング等を追加していく
